...
│   ├── es.json                       # Spanish — source language, always complete
│   ├── en.json
│   └── pt.json
│
...
All three files (`messages/es.json`, `messages/en.json`, `messages/pt.json`) must have identical key structures. Spanish is always the source. Create English and Portuguese translations from Spanish.

Namespace by page section:
```json
{
...
**Secrets** (API keys, tokens, credentials) must never be in `.env.local` or in `defineFunction().environment`. Use AWS Parameter Store or AWS Secrets Manager instead, and access them from inside the Lambda handler using the AWS SDK.

### Production and staging environments

Environment variables for Lambda functions in production/staging are managed in two ways:
...
2. **Secrets**: Store in AWS Parameter Store (SSM) as `SecureString`. Access from the handler using `@aws-sdk/client-ssm`:
   ```typescript
   ...
   ```
   Grant `ssm:GetParameter` in `backend.ts` via `addToRolePolicy`.
...
- [ ] All three `messages/*.json` files have identical key structures
...
- [ ] No secrets in `defineFunction().environment` — secrets go in SSM Parameter Store
